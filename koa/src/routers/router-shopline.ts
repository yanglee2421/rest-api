...
// ** Router
export const shopline = new Router();

// ** Endpoints

// Entry
shopline.get("/", async (ctx, next) => {
  await next();
  ctx.body = ctx.query;
...
shopline.get("/callback", async (ctx, next) => {
  await next();
...
shopline.get("/:id", async (ctx, next) => {
  await next();
  ctx.body = ctx.params;
});
shopline.post("/", async (ctx, next) => {
  await next();
